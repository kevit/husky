 ...
 *****************************************************************************
 *   $Id: tparser.c,v 1.73 2015/09/03 17:49:25 gerritkuehn Exp $
 */
...
int testpath(const char *s, const char *t1, const char *t2, const char *t3)
{
  ...
  if(s)
  {
    ...
    if(!
       (strncmp(s, "./", 2) && strncmp(s, "../", 3) && strncmp(s, ".\\", 2)
        && strncmp(s, "..\\", 3)))
    {
...
int testPathsAndFiles()
{
  ...
  for(i = 0; i < config->netMailAreaCount; i++)
    rc +=
        testpath(config->netMailAreas[i].fileName, "Netmailarea",
                 config->netMailAreas[i].areaName,
                 config->netMailAreas[i].msgbType ==
                 MSGTYPE_SDM ? "pathname" : "filename");

  for(i = 0; i < config->echoAreaCount; i++)
    rc +=
        testpath(config->echoAreas[i].fileName, "Echoarea",
                 config->echoAreas[i].areaName,
                 config->echoAreas[i].msgbType ==
                 MSGTYPE_SDM ? "pathname" : "filename");

  for(i = 0; i < config->localAreaCount; i++)
    rc +=
        testpath(config->localAreas[i].fileName, "Localarea",
                 config->localAreas[i].areaName,
                 config->localAreas[i].msgbType ==
                 MSGTYPE_SDM ? "pathname" : "filename");
  ...
  for(i = 0; i < config->robotCount; i++)
  {
    ...
    rc +=
        testpath(config->robot[i]->newAreaRefuseFile, "newAreaRefuseFile",
                 NULL, NULL);
    rc +=
        testpath(config->robot[i]->autoCreateFlag, "autoCreateFlag", NULL,
                 NULL);
  ...
  for(i = 0; i < config->linkCount; i++)
  {
    ...
    rc +=
        testpath(config->links[i]->fileBox, "Link", config->links[i]->name,
                 "fileBox");
  }
...
const char *testAddr(ps_addr addr)
{
  ...
  {
    return
        "Error: FTN network number must be 16-bit positive number (32767 max) or -1";
  }
  if(addr->node < -1 /*|| addr->node > 32767 */ )
  {
    return
        "Error: FTN node number must be 16-bit positive number (32767 max), zero or -1";
  }
  if(addr->point < -1 /*|| addr->point > 32767 */ )
  {
    return
        "Error: FTN point number must be 16-bit positive number (32767 max), zero or -1";
  }
  if((addr->net == -1) || (addr->node == -1) || (addr->point == -1))
  {
    if((addr->net != -1) || (addr->node != -1)
       || (addr->point && (addr->point != -1)))
    {
      static char s[] =
...
void printAddr(ps_addr addr)
{
  if(addr)
  {
    ...
    {
      if(addr->point)
        printf(" %d:%d/%d.%d@%s ", addr->zone, addr->net, addr->node,
               addr->point, addr->domain);
      else
        printf(" %d:%d/%d@%s ", addr->zone, addr->net, addr->node,
               addr->domain);
    }
    else
    {
      if(addr->point)
        printf(" %d:%d/%d.%d ", addr->zone, addr->net, addr->node,
               addr->point);
      else
...
int printRobot(ps_robot robot)
{
  ...
  if(robot->reportsAttr || robot->reportsFlags)
  {
    ...
    printf("  reportsAttr %s%s%s\n", attrs ? strUpper(attrs) : "",
           attrs ? " " : "", robot->reportsFlags ? robot->reportsFlags : "");
    nfree(attrs);
  }
  ...
  for(i = 0; i + 1 < config->linkCount; i++)
  {
    ...
    for(j = i + 1; j < config->linkCount; j++)
    {
      if(addrComp(config->links[i]->hisAka, config->links[j]->hisAka) == 0)
      {
        ...
        {
          printf("Warning: duplicate definition of link %s ",
                 config->links[i]->name);
          printAddr(&(config->links[i]->hisAka));
          printf("\n");
          continue;
    ...
    if(config->links[i]->areafix.autoCreateFile)
    {
      ...
      if(k < 0)
      {
        ...
        printf(" areafixAutoCreateFile '%s': %s\n",
               config->links[i]->areafix.autoCreateFile, strerror(errno));
        exit(-1);
      }
      else
        close(k);
    ...
    if(config->links[i]->filefix.autoCreateFile)
    {
      ...
      if(k < 0)
      {
        ...
        printf(" filefixAutoCreateFile '%s': %s\n",
               config->links[i]->filefix.autoCreateFile, strerror(errno));
        exit(-1);
      }
      else
        close(k);
  ...
  for(i = 0, cb = &(config->carbons[0]); i < config->carbonCount; i++, cb++)
  {
    ...
    else
    {
      ...
      if(cb->areaName)
      {
        ...
        {
          printf(" !!! \"%s\" wanted !!! using \"%s\"", cbaName,
                 cb->area->areaName);
        }
        else
        {
...
void printRemaps(s_fidoconfig * config)
{
  ...
  for(i = 0; i < config->remapCount; i++)
  {
    printf("ToName: \"%s\" and ToAddress:",
           sstrlen(config->remaps[i].toname) ? config->remaps[i].
           toname : "<any name>");
...
#include "../cvsdate.h"
  ...
  for(k = 1; k < argc; k++)
  {
    ...
    else if(stricmp(argv[k], "--help") == 0 || argv[k][1] == 'h'
            || cfgFile != NULL)
    {
      usage();
      return 0;
  ...
  if(config != NULL)
  {
    ...
    for(i = 0; i < config->addrCount; i++)
    {
      if(config->addr[i].domain[0])
        printf("Addr: %u:%u/%u.%u@%s\n", config->addr[i].zone,
               config->addr[i].net, config->addr[i].node,
               config->addr[i].point, config->addr[i].domain);
      else
        printf("Addr: %u:%u/%u.%u\n", config->addr[i].zone,
               config->addr[i].net, config->addr[i].node,
               config->addr[i].point);
    ...
    switch (config->bundleNameStyle)
    {
      ...
      default:
        printf
            ("Warning: BundleNameStyle is UNKNOWN! Please update tparser!\n");
        break;
    ...
    if(config->ignoreSeenCount)
    {
      ...
      {
        printf(" %u/%u", config->ignoreSeen[i].net,
               config->ignoreSeen[i].node);
      }
      printf("\n");
    }
    ...
    switch (config->LinkWithImportlog)
    {
      ...
      default:
        printf
            ("Internal error: Unknown value #%d for LinkWithImportLog!\n",
    ...
    for(i = 0; i < config->unpackCount; i++)
    {
      printf("UnPacker:  Call: %s Offset %d Match code ",
             config->unpack[i].call, config->unpack[i].offset);
    ...
    if(hpt == 0)
    {
      ...
      for(i = 0; i < config->execonfileCount; i++)
      {
        printf("ExecOnFile: Area %s File %s Call %s\n",
               ...
               config->execonfile[i].filename, config->execonfile[i].command);
      }
    }
